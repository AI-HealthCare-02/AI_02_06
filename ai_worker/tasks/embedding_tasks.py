...
async def run_initial_embedding_task(json_path: str = "ai_worker/data/medicines.json"):
    """
    ...
    await conn.execute_script("ALTER TABLE medicine_info ALTER COLUMN embedding TYPE vector(1536) USING embedding::vector;")
    ...
    for med in medicines:
        try:
            ...
            input_text = f"약품명: {name}\n효능: {med.get('efficacy', '')}\n부작용: {med.get('side_effects', '')}\n주의사항: {med.get('precautions', '')}"
            ...
            # Raw SQL로 저장 (UPSERT)
            sql = """
                INSERT INTO medicine_info (medicine_name, category, efficacy, side_effects, precautions, embedding, created_at, updated_at)
            ...
            await conn.execute_query(
                sql, 
                [name, med.get('category'), med.get('efficacy'), med.get('side_effects'), med.get('precautions'), str(embedding_vector)]
            )
            success_count += 1
