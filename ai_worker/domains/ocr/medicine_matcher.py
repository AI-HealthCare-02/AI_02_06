...
async def _match_clean_name(repo: MedicineInfoRepository, pure_name: str, strength: str, item_type: str, matched: list[ExtractedMedicine], seen_names: set[str]) -> bool:
...
async def _extract_medicines_with_llm(raw_text: str) -> list[dict]:
    ...
    3. 텍스트에 용량(예: 500mg, 100밀리그램, 0.5g 등)이 있다면, 반드시 약품명과 분리해서 추출해. (예: 타이레놀정500mg -> 이름: 타이레놀정, 용량: 500mg)
    4. 응답은 각 항목의 순수 이름(name), 용량(strength), 유형(type: "의약품" 또는 "영양제")을 포함한 JSON 객체 배열 형태로 반환해.
