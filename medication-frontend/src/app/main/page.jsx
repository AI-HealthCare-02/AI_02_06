...
import api from '@/lib/api' // main의 깔끔한 경로 선택
      ...
      <main className="max-w-[1400px] mx-auto w-full px-8 py-12 min-h-screen bg-slate-50 relative overflow-x-hidden">
        ...
        <div className="grid md:grid-cols-12 gap-8">
          <div className="md:col-span-8 w-full h-full">
            <div className="bg-white rounded-[40px] shadow-sm p-10 border border-white/50 w-full h-full">
              ...
              <div className="w-full bg-gray-100 rounded-full h-4 mb-12 overflow-hidden shadow-inner">
                <div className="bg-gradient-to-r from-blue-400 to-blue-600 h-4 rounded-full" style={{width: '66%'}} />
              </div>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {todayMeds.map((med, i) => (
                  <div key={i} onClick={() => router.push('/medication')}
                    className="flex flex-col justify-between p-6 rounded-[32px] bg-slate-50/50 hover:bg-white hover:shadow-xl transition-all border border-transparent hover:border-blue-100 cursor-pointer min-h-[140px]">
                    <div className="flex justify-between items-start w-full">
                      <span className="text-gray-400 text-xs font-black">{med.time}</span>
                      {med.done ? <span className="w-8 h-8 bg-green-500 rounded-full flex items-center justify-center text-white text-sm">✓</span> : <span className="w-8 h-8 bg-white text-blue-500 rounded-full flex items-center justify-center text-xs border-2 border-blue-100">...</span>}
                    </div>
                    <span className={`text-lg font-black mt-4 ${med.done ? 'text-gray-300 line-through' : 'text-gray-800'}`}>{med.name}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="md:col-span-4 flex flex-col space-y-8 w-full h-full">
            <div onClick={() => setShowChat(true)} className="flex-1 bg-blue-500 rounded-[40px] p-10 text-white shadow-2xl cursor-pointer active:scale-[0.96] transition-all relative overflow-hidden group min-h-[220px]">
            ...
            <div onClick={() => router.push('/ocr')} className="flex-1 bg-white rounded-[40px] p-10 border border-gray-100 shadow-sm cursor-pointer active:scale-[0.96] transition-all relative overflow-hidden group min-h-[220px] flex flex-col justify-center">
          ...
          <div className="md:col-span-6 w-full h-full">
            <div onClick={() => router.push('/challenge')} className="bg-white rounded-[40px] shadow-sm p-10 border border-gray-100 cursor-pointer h-full">
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-xl font-black text-gray-900 flex items-center gap-3">🏆 챌린지 현황</h2>
              </div>
              <div className="flex items-center gap-6 mb-8">
                <div className="bg-orange-50 w-20 h-20 rounded-[28px] flex items-center justify-center text-4xl border border-orange-100">🔥</div>
                <div className="flex-1">
                  <span className="font-black text-xl text-gray-800">{challenge.title}</span>
                  <p className="text-orange-500 text-sm font-black mt-1.5">{challenge.days}일째 성공!</p>
                </div>
              </div>
          ...
          <div className="md:col-span-6 w-full h-full">
            <div className="bg-white rounded-[40px] shadow-sm p-10 border border-gray-100 h-full w-full">
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-xl font-black text-gray-900 flex items-center gap-3">📄 최근 처방전</h2>
              </div>
              <div className="bg-slate-50/80 rounded-[32px] p-6 flex items-center justify-between border border-gray-100 hover:bg-white transition-all cursor-pointer group">
                <div className="flex items-center gap-5">
                  <div className="bg-white w-16 h-16 rounded-[24px] flex items-center justify-center text-3xl">📄</div>
                  <div>
        ...
        <div className="md:hidden fixed bottom-0 left-0 w-full bg-white/90 backdrop-blur-lg border-t border-gray-100 flex py-5 px-8 z-40 rounded-t-[40px]">
          <button onClick={() => router.push('/main')} className="flex-1 flex flex-col items-center gap-2">🏠<span className="text-[10px] font-black text-blue-500 uppercase tracking-widest">Home</span></button>
          <button onClick={() => router.push('/mypage')} className="flex-1 flex flex-col items-center gap-2"><span className="opacity-40">👤</span><span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">My</span></button>
        </div>
      </main>
    </>
  )
