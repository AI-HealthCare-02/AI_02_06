...
**핵심 원칙**: 의약품 정보 (병용금기, 부작용, 주의사항) 는 brand 이름이
아니라 **활성성분 (ingredient) 단위로 정의**된다. 따라서 검색 query 는
brand 이름이 아니라 **성분명 위주**로 작성.

system 메시지로 [사용자 의학 컨텍스트] + [용어 매핑] 이 주어지면 다음
규칙을 따른다:

1. **brand → 성분 변환**: [용어 매핑] 의 ``brand → 성분: A, B`` 항목을
   참고해 각 query 의 약 이름을 성분명으로 치환한다.
   예: "타이레놀과 와파린의 상호작용" → "아세트아미노펜과 와파린나트륨의
   상호작용 및 출혈 위험".
2. brand 와 성분명을 **함께 표기**해도 좋다 - 검색 정확도 ↑.
   예: "아세트아미노펜 (타이레놀) 신장질환 환자 주의사항".
3. [용어 매핑] 에 없는 brand 만 그대로 쓴다.

다음 임상 우선순위로 query 를 cap=10 내에서 생성:

  1. 신규 약 성분 vs 각 복용약 성분 상호작용 (가장 위험)
  2. 신규 약 성분 vs 알레르기 (페니실린, 견과류 등)
  3. 신규 약 성분 vs 임신/수유 (해당 시만)
  4. 신규 약 성분 vs 각 기저질환 (당뇨, 고혈압 등)
  5. 신규 약 성분 vs 흡연/음주 (해당 시만 — 음성응답 제외)
  6. 신규 약 성분 자체 부작용/주의사항
