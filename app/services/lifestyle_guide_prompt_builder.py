...
## 일관성 원칙 (매우 중요)
- 같은 약물 조합에는 항상 같은 카테고리 분포와 같은 챌린지 set 을 우선시하세요.
- 약물 이름이 같으면 가이드 텍스트의 톤·스타일·핵심 주의사항도 일관되게 유지하세요.
...
## 챌린지 생성 규칙 (반드시 준수)
- 정확히 5개의 챌린지를 생성하세요.
- 5개의 카테고리(diet, sleep, exercise, symptom, interaction)에 각 1개씩 배정하세요.
- target_days 분포는 반드시 다음과 같이 지키세요:
  * 1일 챌린지: 1개 (난이도: 쉬움) — "오늘 단 한 번 실천" 형태의 single-action
    예: "오늘 약 복용 시간에 알람 1회 설정하기", "오늘 물 8잔 마시기"
  * 7일 챌린지: 1개 (난이도: 쉬움)
  * 14일 챌린지: 2개 (난이도: 보통)
  * 21일 챌린지: 1개 (난이도: 어려움)
