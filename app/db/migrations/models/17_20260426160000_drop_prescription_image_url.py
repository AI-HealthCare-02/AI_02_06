"""Drop ``medications.prescription_image_url`` — dead column 정리.

운영 데이터 점검 결과 66 row 모두 NULL 이었고, 신규 OCR 흐름은 이미지를
S3 등에 영구 저장하지 않고 ai-worker 가 RQ payload bytes 로 직접 처리하므로
이 컬럼이 다시 채워질 가능성도 없음. 모델/DTO/repository/service 에서
필드를 제거하는 변경과 함께 컬럼도 DROP 한다.

downgrade 는 동일 정의로 ADD 하지만 데이터는 복구되지 않음 (None) — 운영
환경에서 되돌릴 일이 없도록 신중히 사용.
"""
...
async def upgrade(db: BaseDBAsyncClient) -> str:
    return """
        ALTER TABLE "medications"
            DROP COLUMN IF EXISTS "prescription_image_url";
    """
...
# ── MODELS_STATE ────────────────────────────────────────────────────
# raw SQL only 마이그레이션. aerich 가 init 시 마지막 파일의 model_state 를
# 검증하므로 직전 #16 의 정상 스냅샷을 그대로 재사용한다.
MODELS_STATE = (
