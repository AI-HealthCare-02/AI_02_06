"""Add status + processed_at to lifestyle_guides for async generation.

기존: ``POST /lifestyle-guides/generate`` 가 OpenAI 호출 (5~30초) 을 동기로
대기 -> FastAPI worker blocking. OCR 비동기화 패턴을 그대로 적용해 RQ
``ai`` 큐의 ``generate_guide_job`` 으로 위임한다.

본 마이그레이션은 그 전제로 ``lifestyle_guides`` 테이블에 처리 상태를
추적할 컬럼 두 개를 추가한다.
...
기존 row 처리: 이미 content 가 채워진 상태 -> ``status='ready'`` 로 backfill.
"""
...
async def upgrade(db: BaseDBAsyncClient) -> str:
    return """
        ...
        COMMENT ON COLUMN "lifestyle_guides"."status" IS
            'pending / ready / failed — ai-worker generate_guide_job 처리 단계';
        COMMENT ON COLUMN "lifestyle_guides"."processed_at" IS
            'ai-worker 처리 완료 시각 (status=ready/failed 일 때 채워짐)';

        -- 기존 row backfill: content 가 이미 채워진 상태이므로 ready 로.
        UPDATE "lifestyle_guides"
            SET "status" = 'ready',
...
# ── MODELS_STATE ────────────────────────────────────────────────────
# aerich format requirement. #15 는 raw SQL only (ALTER TABLE ADD COLUMN) 이며
# 모델 정의는 별도 commit 으로 갱신된다. 직전 #14 의 스냅샷을 그대로 재사용.
MODELS_STATE = (
