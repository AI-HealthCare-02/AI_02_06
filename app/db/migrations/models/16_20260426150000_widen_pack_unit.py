"""Widen ``medicine_info.pack_unit`` from VARCHAR(256) to VARCHAR(2048).

식약처 포장단위 (PACK_UNIT) 가 일부 품목에서 256 자를 초과해 INSERT/UPDATE
가 실패하는 사례가 발생함. 컬럼 폭만 늘리는 단순 ALTER — NOT NULL 제약은
변경 없음 (현재도 NULL 허용).

추가 컬럼 없음. 기존 데이터 손실 없음 (확장만).
"""
...
async def upgrade(db: BaseDBAsyncClient) -> str:
    return """
        ALTER TABLE "medicine_info"
            ALTER COLUMN "pack_unit" TYPE VARCHAR(2048);
    """


async def downgrade(db: BaseDBAsyncClient) -> str:
    # 256 자를 초과하는 row 가 있으면 truncate 되므로 운영 시 주의.
    return """
        ALTER TABLE "medicine_info"
            ALTER COLUMN "pack_unit" TYPE VARCHAR(256);
    """


# ── MODELS_STATE ────────────────────────────────────────────────────
# raw SQL only 마이그레이션이라 신규 모델 추가는 없음. aerich 가 init 시
# 마지막 파일의 model_state 를 검증하므로 직전 #15 의 정상 스냅샷을 그대로 재사용.
MODELS_STATE = (
